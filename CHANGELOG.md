...
- **Install-time invocation-policy gate.** `validateGeneratedPlan` fails with `ERR_CODEX_INVOCATION_POLICY_PLAN` unless the planned `agents/openai.yaml` actually disables implicit invocation, so a corrupted template cannot silently install a route Codex may invoke on its own. Codex reads a missing, permissive, *or schema-rejected* file as `allow_implicit_invocation: true`, so the gate requires the exact generated schema — one top-level `policy:` mapping whose only key is the plain scalar `allow_implicit_invocation: false`. A deeper `policy.<other>.allow_implicit_invocation` leaves the real field absent and is rejected, as is any unsupported top-level or policy field. The error message names the actual defect rather than always pointing at the policy line (`lib/install.js`).
- **Strict YAML block-mapping reader for that gate.** `lib/yaml-block-mapping.js` parses the whole document before the setting is read, so a duplicate key or a syntax error later in the file cannot hide behind an earlier valid value. It accepts only the subset the generator emits and returns null for anything else: sequences, flow style, document markers, tab indentation, anchors, duplicate keys, YAML-non-printable characters, and the `U+0085` / `U+2028` / `U+2029` line separators (which a real YAML reader would treat as breaks, letting a comment smuggle a second `policy:` block past a line-oriented scan). Trimming is ASCII-space-only, so Unicode whitespace cannot turn a contaminated value back into a clean `false`.
- **Generated-file-path validation for Codex plans.** `ERR_CODEX_GENERATED_PATH_PLAN` rejects a plan whose file paths are absent, absolute, unnormalized, or unsafe (`.`/`..`/empty segments, `:`, trailing dot or space), or that collide on a case-insensitive filesystem; the invocation-policy gate additionally requires exactly one `agents/openai.yaml` entry. Without this, an aliased or duplicate entry could land on the validated file's path while escaping validation, because the directory writer writes entries in order (`lib/install.js`).
...
- **Generated routes and runtime guidance name the explicit invocation.** Every generated route's invocation grammar, `Full form:` line, blocked-output template, and r2p rerun guidance now carry the platform prefix, and every r2p `nextAction` derives it from the persisted `runtimePlatform` instead of naming a bare route the user cannot type. `explicitRouteInvocation` renders `$review-fix-*` for Codex, `/review-fix-*` for Claude Code, Gemini, and opencode, and both syntaxes when no platform is known (for example while parsing input). Internal `drfx workflow` arguments keep bare route names (`lib/routes.js`, `lib/generator.js`, `lib/input.js`, `lib/target-context.js`, `lib/workflow/`, `templates/fragments/`, `shared/`).
...
- Route text and r2p `nextAction` strings now include the `$` or `/` prefix. Anything matching those strings exactly needs updating; the machine-readable `blockingReason` and `statusReason` enums are unchanged.
