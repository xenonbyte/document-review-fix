...
// as `true` when the key is absent, so a present-but-wrong file silently re-enables
// implicit invocation. Read the value out of the `policy:` block instead of trusting
// the path: only `allow_implicit_invocation` as a DIRECT child of a top-level `policy:`
// sets the field, so a deeper `policy.<other>.allow_implicit_invocation` must not
// satisfy the gate. This is a fail-closed check on drfx's own generated artifact, not a
// general YAML reader: the block-mapping subset the generator emits is recognized, and
// anything else (flow style, tab indentation, quoted or non-lowercase values, a
// non-top-level `policy:`) fails closed rather than being interpreted.
function codexPolicyDisablesImplicitInvocation(content) {
  if (typeof content !== 'string') return false;
  const lines = content.split('\n');
  const policyIndex = lines.indexOf('policy:');
  if (policyIndex === -1) return false;

  // The first more-indented line fixes the indentation of the block's direct children.
  let childIndent = null;
  for (let index = policyIndex + 1; index < lines.length; index += 1) {
    const line = lines[index];
    if (line.trim() === '') continue;
    const indent = /^[ \t]*/.exec(line)[0];
    // YAML forbids tab indentation; an unindented line ends the block before the key.
    if (indent === '' || indent.includes('\t')) return false;
    if (childIndent === null) childIndent = indent;
    // Deeper than a direct child: this key belongs to a nested mapping, not to `policy`.
    if (indent.length > childIndent.length) continue;
    // Shallower than the established child indent: malformed, or the block already ended.
    if (indent.length < childIndent.length) return false;
    const match = /^allow_implicit_invocation:[ \t]*(\S+)[ \t]*$/.exec(line.slice(indent.length));
...
function validateCodexInvocationPolicy(generated) {
  ...
  const metadata = (generated.files || []).find((file) => file.relativePath === CODEX_AGENTS_METADATA_PATH);
  if (!metadata) {
    fail(
  ...
  if (!codexPolicyDisablesImplicitInvocation(metadata.content)) {
    fail(
      'ERR_CODEX_INVOCATION_POLICY_PLAN',
      `Codex ${CODEX_AGENTS_METADATA_PATH} must set policy.allow_implicit_invocation to false`
    );
  }
}
